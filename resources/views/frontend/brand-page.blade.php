...
@section('title', $brand_products->name)
@section('content')

    <div class="flex flex-col py-6 border-b">
        <div class="text-base font-semibold">
            <a href="{{ route('frontend.home_page') }}" class="hover:text-theme"><i class="fa fa-home"></i></a>&nbsp;
            <a href="{{ route('frontend.brand.page', $brand_products->id) }}">{{ $brand_products->name }}</a>

        </div>
    </div>
    @if(count($brand_products->product) > 0)

    <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 items-center bg-white px-4 md:px-0 py-6">

    @foreach($brand_products->product as $fetch_brand_product)
        <!-- Single Product -->
            <div class="product-card flex flex-col text-center border-[1px] border-theme rounded-lg group">
                <a href="{{ route('frontend.product.details', $fetch_brand_product->slug) }}" class=" product-img rounded-tl-lg rounded-tr-lg overflow-hidden relative">
                    <img class="group-hover:scale-125 duration-500 w-full" src="{{ asset('/storage/product/'.$fetch_brand_product->feature_image) }}" alt="{{ $fetch_brand_product->feature_image }}">
                    @if($fetch_brand_product->discount !== null)
                        <div class="absolute top-0 right-0 px-2 bg-theme rounded-bl-lg">
                            <span class=" text-white text-xs">
                                <span class="font-bold">Flat {{$fetch_brand_product->discount}}% Off</span>
                            </span>
                        </div>
                    @endif
                    ...
                    <div class="cart-favorite flex flex-col">
                        <!-- Wishlist Icon -->
                        <button id="{{ $fetch_brand_product->id }}" onclick="add_to_favorite(event, this.id)" class="wishlist_button absolute top-6 -right-10 group-hover:right-0 duration-300">
                        ...
                        <!-- Wishlist Icon -->
                        <!-- Add to cart icon -->
                        <button id="{{ $fetch_brand_product->id }}" onclick="add_to_carts(event, this.id)" class="add_to_cart_button absolute top-14 -right-20 group-hover:right-0 duration-300">
                ...
                <div class="product-price-info flex flex-col gap-2 py-2 items-center min-h-[125px]">
                    <a class="min-h-[44px]" href="{{ route('frontend.product.details', $fetch_brand_product->slug) }}"><h2 class="text-[14px] group-hover:text-theme font-semibold duration-500 px-2">{{ Str::limit($fetch_brand_product->title, 40) }}</h2></a>
                    <div class="product_price">
                        @if($fetch_brand_product->discount !== null)
                            <div class="flex flex-row gap-2 items-center justify-center">
                                <div class="flex flex-row justify-center items-center gap-x-0.5 text-[12px] font-bold line-through">
                                    <span> TK</span><span>{{$fetch_brand_product->unit_price}}</span>
                                </div>

                                <div class="text-[13px] text-theme font-bold flex items-center">
                                    <span> TK </span><span>&nbsp;{{$fetch_brand_product->discount_price}}</span>
                                </div>
                            </div>
                        @else
    ...
    @else
        <div class="text-center py-6 my-8">
            <h2 class="text-xl">There is no brand products!</h2>
        </div>
    @endif
@endsection
