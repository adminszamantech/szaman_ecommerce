...
<div class="hot-products flex flex-col gap-3 my-4">
    <div class="flex flex-row justify-center items-center shadow-xl bg-theme py-2">
        <div class="flex flex-row items-center justify-center">
            ...
            <h2 class="text-base md:text-2xl text-center font-semibold text-white px-4">HOT DEAL</h2>
            <img class="w-10" src="{{ asset('frontend/assets/img/fire.gif') }}" alt="">
        </div>
    </div>
    <div class="relative">
        <div class="swiper hotDealSlider">
                ...
                <!-- Single Product -->
                    <div class="swiper-slide px-1 py-3">
                        <div class="product-card flex flex-col text-center shadow-xl rounded-lg group">
                            <a href="{{ route('frontend.product.details', $hot_deal->slug) }}" class=" product-img rounded-tl-lg rounded-tr-lg overflow-hidden relative">
                                <img class="group-hover:scale-125 duration-500 w-full" src="{{ asset('/storage/product/'.$hot_deal->feature_image) }}" alt="{{ $hot_deal->feature_image }}">
                                @if($hot_deal->discount !== null)
                                    <div class="absolute top-0 right-0 px-2 bg-theme rounded-bl-lg">
                            <span class=" text-white text-xs">
                                <span class="font-bold">Flat {{$hot_deal->discount}}% Off</span>
                            </span>
                                    </div>
                                @endif
                                ...
                                <div class="cart-favorite flex flex-col">
                                    <!-- Wishlist Icon -->
                                    <button id="{{ $hot_deal->id }}" onclick="add_to_favorite(event, this.id)" class="wishlist_button absolute top-6 -right-10 group-hover:right-0 duration-300">
                                    ...
                                    <!-- Wishlist Icon -->
                                    <!-- Add to cart icon -->
                                    <button id="{{ $hot_deal->id }}" onclick="add_to_carts(event, this.id)" class="add_to_cart_button absolute top-14 -right-20 group-hover:right-0 duration-300">
                            ...
                            <div class="product-price-info flex flex-col gap-2 py-2 items-center min-h-[125px]">
                                <a class="min-h-[44px]" href="{{ route('frontend.product.details', $hot_deal->slug) }}"><h2 class="text-[14px] group-hover:text-theme font-semibold duration-500 px-2">{{ Str::limit($hot_deal->title, 40) }}</h2></a>
                                <div class="product_price">
                                    @if($hot_deal->discount !== null)
                                        <div class="flex flex-row gap-2 items-center justify-center">
                                            <div class="flex flex-row justify-center items-center gap-x-0.5 text-[12px] font-bold line-through">
                                                <span><b>৳</b>{{ number_format($hot_deal->unit_price) }}</span>
                                            </div>

                                            <div class="text-[13px] text-theme font-bold flex items-center">
                                                <span><b>৳</b>{{ number_format($hot_deal->discount_price) }}</span>
                                            </div>
                                        </div>
                                    @else
            ...
            <span class="hotdeal-swiper-button-next absolute top-[40%] -left-0 z-[9999]"><i class="border border-theme rounded-full px-2 bg-theme pt-[2px] pb-[2px] fa-solid fa-arrow-left text-base md:text-2xl text-white"></i></span>
            <span class="hotdeal-swiper-button-prev absolute top-[40%] -right-0 z-[9999]"><i class="border border-theme hover:bg-theme hover:text-white rounded-full px-2 pt-[2px] pb-[2px] fa-solid fa-arrow-right text-base md:text-2xl bg-theme text-white"></i></span>
        </div>
    </div>
