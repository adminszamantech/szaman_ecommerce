...
<div class="best-selling-products flex flex-col gap-3 my-4">
    ...
    <div class="relative">
        <div class="swiper bestSellingSlider">
                ...
                <!-- Single Product -->
                    <div class="swiper-slide px-1 py-3">
                        <div class="product-card flex flex-col text-center shadow-xl rounded-lg group">
                            <a href="{{ route('frontend.product.details', $best_sell->slug) }}" class=" product-img rounded-tl-lg rounded-tr-lg overflow-hidden relative">
                                <img class="group-hover:scale-125 duration-500 w-full" src="{{ asset('/storage/product/'.$best_sell->feature_image) }}" alt="{{ $best_sell->feature_image }}">
                                @if($best_sell->discount !== null)
                                    <div class="absolute top-0 right-0 px-2 bg-theme rounded-bl-lg">
                            <span class=" text-white text-xs">
                                <span class="font-bold">Flat {{$best_sell->discount}}% Off</span>
                            </span>
                                    </div>
                                @endif
                                ...
                                <div class="cart-favorite flex flex-col">
                                    <!-- Wishlist Icon -->
                                    <button id="{{ $best_sell->id }}" onclick="add_to_favorite(event, this.id)" class="wishlist_button absolute top-6 -right-10 group-hover:right-0 duration-300">
                                    ...
                                    <!-- Wishlist Icon -->
                                    <!-- Add to cart icon -->
                                    <button id="{{ $best_sell->id }}" onclick="add_to_carts(event, this.id)" class="add_to_cart_button absolute top-14 -right-20 group-hover:right-0 duration-300">
                            ...
                            <div class="product-price-info flex flex-col gap-2 py-2 items-center min-h-[125px]">
                                <a class="min-h-[44px]" href="{{ route('frontend.product.details', $best_sell->slug) }}"><h2 class="text-[14px] group-hover:text-theme font-semibold duration-500 px-2">{{ Str::limit($best_sell->title, 40) }}</h2></a>
                                <div class="product_price">
                                    @if($best_sell->discount !== null)
                                        <div class="flex flex-row gap-2 items-center justify-center">
                                            <div class="flex flex-row justify-center items-center gap-x-0.5 text-[12px] font-bold line-through">
                                                <span><b>৳</b>{{ number_format($best_sell->unit_price) }}</span>
                                            </div>

                                            <div class="text-[13px] text-theme font-bold flex items-center">
                                                <span><b>৳</b>{{ number_format($best_sell->discount_price) }}</span>
                                            </div>
                                        </div>
                                    @else
