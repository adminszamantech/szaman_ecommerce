...
<div class="related_product_title bg-theme text-white shadow-xl py-1 md:py-2 text-center my-4">
    ...
    @foreach($related_products as $related_product)
        <!-- Single Product -->
            <div class="product-card flex flex-col text-center shadow-xl rounded-lg group">
                <a href="{{ route('frontend.product.details', $related_product->slug) }}" class=" product-img rounded-tl-lg rounded-tr-lg overflow-hidden relative">
                    <img class="group-hover:scale-125 duration-500 w-full" src="{{ asset('/storage/product/'.$related_product->feature_image) }}" alt="{{ $related_product->feature_image }}">
                    ...
                    <div class="cart-favorite flex flex-col">
                        <!-- Wishlist Icon -->
                        <button id="{{ $related_product->id }}" onclick="add_to_favorite(event, this.id)" class="wishlist_button absolute top-6 -right-10 group-hover:right-0 duration-300">
                ...
                <div class="product-price-info flex flex-col gap-2 py-2 items-center min-h-[125px]">
                    <a class="min-h-[44px]" href="{{ route('frontend.product.details', $related_product->slug) }}"><h2 class="text-[14px] group-hover:text-theme font-semibold duration-500 px-2">{{ Str::limit($related_product->title, 40) }}</h2></a>
                    <div class="product_price">
                        @if($related_product->discount !== null)
                            <div class="flex flex-row gap-2 items-center justify-center">
                                <div class="flex flex-row justify-center items-center gap-x-0.5 text-[12px] font-bold line-through">
                                    <span><b>৳</b>{{ number_format($related_product->unit_price) }}</span>
                                </div>

                                <div class="text-[13px] text-theme font-bold flex items-center">
                                    <span><b>৳</b>{{ number_format($related_product->discount_price) }}</span>
                                </div>
                            </div>
                        @else
